...
class MockCatenaServiceImpl : public CatenaServiceImpl {
    public:
        ...
        MOCK_METHOD(void, RequestUpdateSubscriptions, (ServerContext*, catena::UpdateSubscriptionsPayload*, ServerAsyncWriter<catena::DeviceComponent_ComponentParam>*, grpc::CompletionQueue*, grpc::ServerCompletionQueue*, void*), ());
};
//Fixture
...
TEST_F(UpdateSubscriptionsTest, UpdateSubscriptions_Constructor) {
    ...
    EXPECT_CALL(*service, registerItem(_)).Times(1);
    EXPECT_CALL(*service, authorizationEnabled()).WillOnce(Return(false));
    EXPECT_CALL(*service, RequestUpdateSubscriptions(_, _, _, _, _, _)).Times(1);
