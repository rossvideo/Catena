...
func writeHTTPResult(w http.ResponseWriter, res catena.StatusResult) {
    ...
    case res.Status >= 200 && res.Status < 300:
        ...
        w.WriteHeader(res.Status)
        internal.WriteResponseJSON(w, res.Payload)
    default:
...
type GetParamValueHandler func(slot int, fqoid string) catena.StatusResult
type SetParamValueHandler func(value any, slot int, fqoid string) catena.StatusResult
...
type Server struct {
    ...
    getParamValue map[int]GetParamValueHandler
    setParamValue map[int]SetParamValueHandler
...
func NewServer(slotList []int) *Server {
    ...
    server := Server{
        ...
        getParamValue: make(map[int]GetParamValueHandler),
        setParamValue: make(map[int]SetParamValueHandler),
...
func (s *Server) RegisterGetParamHandler(slot int, handler GetParamValueHandler) {
    s.mu.Lock()
    defer s.mu.Unlock()
...
func (s *Server) RegisterSetParamHandler(slot int, handler SetParamValueHandler) {
    s.mu.Lock()
    defer s.mu.Unlock()
...
func (s *Server) lookupGetParamValue(slot int) (GetParamValueHandler, bool) {
    s.mu.RLock()
    defer s.mu.RUnlock()
    h, ok := s.getParamValue[slot]
    return h, ok
}

func (s *Server) lookupSetParamValue(slot int) (SetParamValueHandler, bool) {
    s.mu.RLock()
    defer s.mu.RUnlock()
    h, ok := s.setParamValue[slot]
    return h, ok
}
    ...
    for _, slot := range s.slotList {
        ...
        s.mux.HandleFunc(prefix+"/asset/", func(w http.ResponseWriter, r *http.Request) {
            ...
            if handler, ok := s.lookupGetAsset(slot); ok {
                // Asset handlers stream directly and return a status
