...
ENV CMAKE_COMMAND="cmake -G Ninja -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -DCONNECTIONS=${CONNECTIONS} -DCOVERAGE=ON -DCMAKE_INSTALL_PREFIX=/usr/local/.local -DCMAKE_EXPORT_COMPILE_COMMANDS=TRUE -DGLOG_LOGGING_DIR=~/Catena/logs -B ~/Catena/${BUILD_TARGET} -S ~/Catena/sdks/cpp"
